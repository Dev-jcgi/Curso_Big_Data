...
```bash
# 1. Crear hive-env.sh desde la plantilla (si no existe)
cp /opt/hive/conf/hive-env.sh.template /opt/hive/conf/hive-env.sh

# 2. Editar AMBOS archivos con tu editor preferido (nano, vim, etc.)
# Agregar o verificar estas líneas:
export JAVA_HOME=/opt/jdk1.8.0_202
export HADOOP_HOME=/opt/hadoop/hadoop-3.4.0
```

> **📝 Nota:** Si Hive está corriendo, sal de la sesión primero ejecutando `exit;` dentro de Hive, luego reinicia después de editar los archivos.
...
Por defecto, Hadoop usa el usuario ficticio "dr.who" para la interfaz web, lo que causa errores de permisos. Esta configuración fuerza el uso del usuario real (hadoop) para todas las operaciones web del HDFS, eliminando el error "Permission denied" en el navegador.
...
> **⚠️ Prerequisito:** Ejecutar estos comandos DESPUÉS de iniciar HDFS (ver PASO 1)
...
### 1.2 Iniciar Servicios Hadoop

```bash
# Si los servicios ya están corriendo, detenerlos primero
stop-all.sh
...
> **💡 Nota:** Los números de proceso (PIDs) serán diferentes en tu sistema. Lo importante es que veas los 5 servicios principales.
