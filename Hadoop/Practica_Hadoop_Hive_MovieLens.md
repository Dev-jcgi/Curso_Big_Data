...
1. [Configuración Crítica](#-paso-0-configuración-crítica)
2. [Iniciar Servicios Hadoop y YARN](#-paso-1-iniciar-servicios-hadoop-y-yarn)
3. [Cargar Dataset desde Archivo .data](#-paso-2-cargar-dataset-desde-archivo-data-a-hdfs)
4. [Crear Tablas y Consultas en Hive](#-paso-3-crear-base-de-datos-y-tablas-en-hive)
5. [Consultas Analíticas con MapReduce](#-paso-4-consultas-analíticas-con-mapreduce-yarn)
6. [Monitoreo de Jobs YARN](#-paso-5-monitoreo-de-jobs-yarn-y-mapreduce)
7. [Exportar Resultados](#-paso-6-exportar-resultados)
8. [Consultas Avanzadas Bonus](#-paso-7-consultas-avanzadas-bonus)

---

## 🛠️ PASO 0: Configuración Crítica

> ⚠️ **IMPORTANTE:** Ejecuta esta configuración ANTES de iniciar cualquier análisis

### 📌 Problemas que Resolveremos

| # | Error Común                     | Causa                          | Solución                                           |
| - | -------------------------------- | ------------------------------ | --------------------------------------------------- |
| 1 | Java 17 incompatible             | Hive no soporta Java > 11      | Forzar Java 8 en archivos de configuración         |
| 2 | "Permission denied: user=dr.who" | Usuario ficticio en web UI     | Configurar `hadoop.http.staticuser.user`          |
| 3 | "Return Code 2" en GROUP BY      | Hive ejecuta en modo local     | Forzar uso de YARN con `SET` commands             |
| 4 | Errores en staging directory     | Falta directorio temporal YARN | Crear y dar permisos a `/tmp/hadoop-yarn/staging` |

---

### A. Configurar Variables de Entorno para Java 8

#### 🔧 Archivos a Modificar:

> ⚠️ **IMPORTANTE:** *_Si los archivos ya estan configurados previamente no hacer cambios_*

**1. Hadoop Environment (`/opt/hadoop/hadoop-3.4.0/etc/hadoop/hadoop-env.sh`):**

```bash
# Abrir el archivo con tu editor favorito
nano /opt/hadoop/hadoop-3.4.0/etc/hadoop/hadoop-env.sh
...
**2. Hive Environment (`/opt/hive/conf/hive-env.sh`):**
...
java -version
# Esperado: java version "1.8.0_202"
```

---

### B. Configurar Usuario Web de Hadoop (Evitar Error "dr.who")
...
Por defecto, Hadoop usa el usuario "dr.who" para operaciones web, causando errores de permisos. Esta configuración usa tu usuario real (hadoop) para todas las operaciones en http://localhost:9870.

---

### C. Preparar Sistema de Archivos

> **⚠️ Nota:** Ejecutar DESPUÉS de iniciar HDFS en el Paso 1

```bash
# Dar permisos completos al filesystem HDFS (raíz)
...
# Crear directorio para nuestros datos MovieLens
hdfs dfs -mkdir -p /datasets/movielens
hdfs dfs -chmod -R 777 /datasets/movielens
```
...
## 🚀 PASO 1: Iniciar Servicios Hadoop y YARN

⏱️ **Tiempo estimado:** 2-3 minutos

### 1.1 Cargar Variables de Entorno en la Sesión Actual

```bash
# Configurar PATH con binarios de Hadoop y Hive
export HADOOP_HOME=/opt/hadoop/hadoop-3.4.0
export HIVE_HOME=/opt/hive
...
which hadoop
# Esperado: /opt/hadoop/hadoop-3.4.0/bin/hadoop

which hive
# Esperado: /opt/hive/bin/hive

which hdfs
# Esperado: /opt/hadoop/hadoop-3.4.0/bin/hdfs
```

---

### 1.2 Detener Servicios Previos (si existen)

```bash
# Detener todos los servicios Hadoop/YARN si ya están corriendo
stop-all.sh
...
### 1.3 Iniciar HDFS (Sistema de Archivos Distribuido)
...
# Verificar que HDFS esté activo
...
**💡 Explicación de los Servicios HDFS:**

| Servicio                    | Función | Responsabilidad                                       |
| --------------------------- | -------- | ----------------------------------------------------- |
| **NameNode**          | Master   | Administra metadatos (nombres, ubicación de bloques) |
| **DataNode**          | Worker   | Almacena bloques de datos reales                      |
| **SecondaryNameNode** | Backup   | Realiza checkpoints del NameNode                      |

---

### 1.4 Iniciar YARN (Gestor de Recursos y Jobs)
...
# Verificar que YARN esté activo
...
**💡 Explicación de los Servicios YARN:**

| Servicio                  | Función | Responsabilidad                                            |
| ------------------------- | -------- | ---------------------------------------------------------- |
| **ResourceManager** | Master   | Gestiona recursos del cluster (CPU, RAM) y distribuye jobs |
| **NodeManager**     | Worker   | Ejecuta contenedores con tasks MapReduce                   |

---

### 1.5 Verificación Final de Todos los Servicios
...
> **✅ CHECKPOINT:** Si ves los 5 servicios (NameNode, DataNode, SecondaryNameNode, ResourceManager, NodeManager), puedes continuar al siguiente paso.

---

### 1.6 Verificar Interfaces Web
...
## 📁 PASO 2: Cargar Dataset desde Archivo .data a HDFS

⏱️ **Tiempo estimado:** 3-5 minutos (depende del tamaño del archivo)

---

### 2.1 Entender el Dataset MovieLens
...
### 2.2 Verificar que el Archivo Existe Localmente
...
### 2.3 Crear Directorio en HDFS para el Dataset
...
### 2.4 Cargar Archivo .data desde Sistema Local a HDFS
...
### 2.5 Verificar que los Datos se Cargaron Correctamente
...
### 2.6 Explorar Distribución de Bloques en HDFS
...
### 2.7 Validación Final con Interfaz Web
...
## 🐝 PASO 3: Crear Base de Datos y Tablas en Hive
...
### 3.2 ⚠️ CONFIGURACIÓN OBLIGATORIA (Evitar Errores)
...
### 3.4 Crear Tabla Externa para los Ratings
...
### 3.5 Verificar que la Tabla se Creó Correctamente
...
### 3.6 Prueba Inicial: Ver los Datos en Hive
...
## 🔄 PASO 4: Consultas Analíticas con MapReduce (YARN)
...
### 4.1 Consulta 1: Distribución de Ratings (1-5 estrellas)
...
### 4.2 Consulta 2: Top 10 Películas Más Calificadas
...
### 4.3 Consulta 3: Top 10 Películas Mejor Calificadas (con mínimo de ratings)
...
### 4.4 Consulta 4: Análisis de Actividad de Usuarios
...
### 4.5 Consulta 5: Análisis Temporal (Ratings por Mes)
...
## 📊 PASO 5: Monitoreo de Jobs YARN y MapReduce
...
### 5.1 Monitoreo desde Interfaz Web YARN
...
### 5.3 Monitoreo desde Interfaz Web HDFS
...
### 5.4 Interpretar el Flujo de un Job MapReduce
...
### 5.5 Diagnóstico de Errores Comunes
...
### 6.1 Exportar a Sistema de Archivos Local (CSV)
...
### 6.2 Exportar Top Películas a CSV con Encabezado
...
### 6.3 Exportar a HDFS (para compartir con otros procesos)
...
### 6.4 Crear Tabla Materializada para Consultas Rápidas
...
### 6.5 Exportar Múltiples Reportes (Script Completo)
...
## 🎓 PASO 7: Consultas Avanzadas (Bonus)

⏱️ **Tiempo estimado:** 15-20 minutos

---

### 7.1 Análisis de Consenso: Películas Polarizantes vs Consensuadas
...
### 7.2 Usuarios Generosos vs Críticos
...
### 7.3 Análisis de Picos de Actividad (Día de la Semana)
...
### 7.4 Películas con Mayor Variabilidad en el Tiempo
...
### 7.5 Matriz de Correlación: Ratings por Usuario Activo
...
### 7.6 Top Películas por Segmento de Rating
...
### 7.7 Análisis de "Long Tail": Distribución de Popularidad
